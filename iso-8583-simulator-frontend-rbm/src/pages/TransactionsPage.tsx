...
const Transactions = () => {
  ...
  const fetchFilterMetadata = async () => {
    try {
      ...
      const data = await apiGet<TransactionFilterMetadata>('/transactions/currentinfotransactions');
      setFilterMetadata(data);
    } catch (error) {
      console.error('Error fetching filter metadata:', error);
      toast({ title: 'Error', description: t.common.failedLoadFilters, variant: 'destructive' });
    } finally {
  ...
  useEffect(() => {
    if (viewMode === 'history') fetchFilterMetadata();
  }, [viewMode]);
  ...
  const fetchTransactions = async () => {
    try {
      ...
      if (filters.authCodeEmpty) params.append("authCodeEmpty", "true");
      if (filters.rrnEmpty) params.append("rrnEmpty", "true");
      if (filters.responseCodeEmpty) params.append("responseCodeEmpty", "true");
      ...
      if (filters.amountFrom) params.append('amountFrom', String(filters.amountFrom));
      if (filters.amountTo) params.append('amountTo', String(filters.amountTo));
      params.append('page', String(currentPage - 1));
      params.append('size', String(itemsPerPage));

      const data = await apiGet<PageResponse<Transaction>>(`/transactions?${params.toString()}`);
  ...
  const displayTransactions = viewMode === 'live' ? liveTransactionsWs : transactions;
  const paginatedTransactions = viewMode === 'history' ? transactions : displayTransactions;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">{t.tx.title}</h1>
          <p className="text-muted-foreground mt-1">{t.tx.monitorSearch}</p>
        </div>
        <div className="flex gap-2">
          <Button variant={viewMode === 'live' ? 'default' : 'outline'} onClick={() => setViewMode('live')} className="gap-2">
            <Radio className={`h-4 w-4 ${viewMode === 'live' ? 'animate-pulse' : ''}`} />
            {t.tx.liveFeed}
          </Button>
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              {t.tx.showing} {transactions.length} {t.tx.of} {totalElements} {t.tx.transactionsLabel}
            </p>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">{t.tx.itemsPerPage}</span>
      ...
      {viewMode === 'history' && totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">
            {t.tx.page} {currentPage} {t.tx.of} {totalPages}
          </span>
